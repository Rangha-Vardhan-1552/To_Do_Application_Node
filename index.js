const todoList = () => {
  ...
  const overdue = () => {
    ...
    const overdue1 = (temp) => {
    if(temp.dueDate<today) return true;
    else return false;
  }

    var dateToday = new Date()
    const today = formattedDate(dateToday)
    const due=all.filter(overdue1);
    return due 
  ...
  const dueToday = () => {
    ...
    var dateToday = new Date()
    const today = formattedDate(dateToday)
    const duetoday=all.filter(Tday);

    function Tday(temp){
        return temp.dueDate===today;
    }
    return duetoday;
  ...
  const dueLater = () => {
    ...
    var dateToday = new Date()
    const today = formattedDate(dateToday)
    const duelater=all.filter(Dlater);

    function Dlater(temp){
        return temp.dueDate>today;
    }
    return duelater;
  }
